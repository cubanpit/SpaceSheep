...
Engine::Engine(
    unsigned int xDim, unsigned int yDim, unsigned int fatness,
...
bool Engine::run_local()
{
  ...
  while (!dead) {
    ...
    for (unsigned short int i=0; i<tmp_dt_uint_bushes and !dead;
        i+=m_dt_uint_sheep) {
      ch = getch();
...
void Engine :: set_bushes_properties (
    unsigned int bushes_w_d, unsigned int bushes_w_tot, unsigned int bushes_w_m,
    unsigned int bushes_w_r, unsigned int bushes_h_m, unsigned int bushes_h_r)
{
...
void Engine :: set_movement_properties (
    unsigned int dt_uint_sheep, char left_mov, char right_mov)
{
